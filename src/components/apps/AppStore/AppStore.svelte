<script lang="ts">
  ...
  function loadTodos(): Todo[] {
    try { return JSON.parse(localStorage.getItem('os.todos') || '[]'); } catch { return []; }
  }
  function saveTodos() {
    try { localStorage.setItem('os.todos', JSON.stringify(todos)); } catch {}
  }
  function addTodo() {
  ...
  function toggleTodo(id: number) {
    // `bind:checked` aktualisiert den Status bereits.
    // Diese Funktion dient nur zum Speichern der Änderung.
    saveTodos();
  }
